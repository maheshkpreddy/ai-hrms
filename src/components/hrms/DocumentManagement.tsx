...
export default function DocumentManagement() {
  ...
  // Sync sidebar sub-item selections to internal component state
  // Mapping: 'doc-list' → all documents view, 'upload-doc' → open upload dialog,
  //          'templates' → templates view (placeholder for future implementation)
  useEffect(() => {
    if (!activeSubItem) return
    if (activeSubItem === 'doc-list') {
      // Show all documents — reset filters and close any open dialog
      setFilterDocType('all')
      setFilterAccess('all')
      setSearchQuery('')
      setAddOpen(false)
      setEditOpen(false)
    } else if (activeSubItem === 'upload-doc') {
      // Open the upload/add document dialog
      setAddOpen(true)
    } else if (activeSubItem === 'templates') {
      // Templates view — placeholder for future implementation
      // Could set an active tab/view state when templates are added
    }
    // Clear the sub-item after reacting so re-clicks still trigger
