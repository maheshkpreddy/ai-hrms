...
const SAMPLE_BRANCHES: BranchMaster[] = [
  {
    ...
    name: 'Mumbai HQ',
    code: 'MUM-HQ',
    company: 'MARQ AI Technologies',
    address: 'BKC, Bandra Kurla Complex, Mumbai',
    geoLocation: '19.0596, 72.8654',
    timezone: 'Asia/Kolkata',
    geofenceRadius: '500m',
  ...
  },
  {
    ...
    company: 'Acme Corp',
    address: 'Connaught Place, New Delhi',
    geoLocation: '28.6329, 77.2200',
    timezone: 'Asia/Kolkata',
    geofenceRadius: '300m',
    branchManager: 'Amit Verma',
    status: 'active',
  },
  {
    id: 'br-4',
  ...
  },
  {
    ...
    geoLocation: '18.5913, 73.7389',
    timezone: 'Asia/Kolkata',
    geofenceRadius: '300m',
    branchManager: 'Sneha Patil',
    status: 'inactive',
  },
]
...
export default function MasterManagement() {
  ...
  const toggleStatus = useCallback((id: string) => {
    const newData = currentData.map((item) =>
      item.id === id ? { ...item, status: (item as any).status === 'active' ? 'inactive' : 'active' } : item
    ) as MasterRecord[]
    updateData(newData)
    toast({ title: 'Status Updated', description: 'Record status has been toggled.' })
  }, [currentData, updateData, toast])
  ...
  }, [deleteConfirm, currentData, updateData, toast])
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
        ...
        <Tabs value={activeTab} onValueChange={handleTabChange}>
          ...
          {/* ── Tab Content (for tab routing, actual content is the shared table above) ── */}
          {TABS.map(tab => (
  ...
  function renderTableHeaders(tab: string) {
    const base = [<TableHead key="name" className="pl-4">Name</TableHead>]
      ...
      case 'companies':
        return (
          <>
            ...
            <TableHead className="hidden lg:table-cell">GST/VAT</TableHead>
    ...
    const renderActions = (id: string) => (
      <div className="flex items-center justify-end gap-1">
        <Button
          ...
          onClick={() => { setEditingRecord(record); setEditOpen(true) }}
          title="Edit"
        >
        ...
        <Button
          variant="ghost"
          size="icon"
          className="h-8 w-8 text-muted-foreground hover:bg-gray-100 hover:text-gray-700 dark:hover:bg-gray-800"
          onClick={() => toggleStatus(id)}
          title="Toggle Status"
        >
          {record.status === 'active' ? <ToggleRight className="h-4 w-4 text-emerald-600" /> : <ToggleLeft className="h-4 w-4" />}
        </Button>
        <Button
          variant="ghost"
          size="icon"
            ...
            <TableCell className="pl-4">
              <div className="flex items-center gap-3">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-[11px] text-muted-foreground truncate">{record.legalName}</p>
                </div>
              </div>
            </TableCell>
            <TableCell><code className="rounded bg-gray-100 px-1.5 py-0.5 text-xs font-mono font-semibold">{record.code}</code></TableCell>
            <TableCell className="hidden lg:table-cell"><span className="text-xs text-muted-foreground">{record.gstVat}</span></TableCell>
  ...
  function renderForm(tab: string, mode: 'add' | 'edit') {
    ...
    switch (tab) {
      case 'companies':
        return <CompanyForm record={record as CompanyMaster | null} mode={mode} />
      case 'branches':
        return <BranchForm record={record as BranchMaster | null} mode={mode} />
      case 'departments':
        return <DepartmentForm record={record as DepartmentMaster | null} mode={mode} />
      case 'designations':
        return <DesignationForm record={record as DesignationMaster | null} mode={mode} />
      case 'grades':
        return <GradeForm record={record as GradeMaster | null} mode={mode} />
      case 'shifts':
        return <ShiftForm record={record as ShiftMaster | null} mode={mode} />
      case 'leave-types':
        return <LeaveTypeForm record={record as LeaveTypeMaster | null} mode={mode} />
      case 'skills':
        return <SkillForm record={record as SkillMaster | null} mode={mode} />
      case 'document-types':
        return <DocumentTypeForm record={record as DocumentTypeMaster | null} mode={mode} />
      default:
        return null
    }
  }
  ...
  function CompanyForm({ record, mode }: { record: CompanyMaster | null; mode: 'add' | 'edit' }) {
    const [form, setForm] = useState<Partial<CompanyMaster>>(
      mode === 'edit' && record
        ? { ...record }
        : { name: '', legalName: '', code: '', gstVat: '', panTanCin: '', registrationNumber: '', industryType: '', address: '', country: 'India', state: '', city: '', currency: 'INR', timezone: 'Asia/Kolkata', financialYear: 'April - March', payrollCycle: 'Monthly', defaultLanguage: 'English', status: 'active' }
    )
    ...
    const handleSave = () => {
      ...
      if (mode === 'add') {
        const newRecord: CompanyMaster = { id: genId(), name: form.name || '', legalName: form.legalName || '', code: form.code || '', gstVat: form.gstVat || '', panTanCin: form.panTanCin || '', registrationNumber: form.registrationNumber || '', industryType: form.industryType || '', logo: '', address: form.address || '', country: form.country || 'India', state: form.state || '', city: form.city || '', currency: form.currency || 'INR', timezone: form.timezone || 'Asia/Kolkata', financialYear: form.financialYear || 'April - March', payrollCycle: form.payrollCycle || 'Monthly', defaultLanguage: form.defaultLanguage || 'English', status: 'active' }
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div className="space-y-2">
            ...
            <Select value={form.industryType || ''} onValueChange={v => setForm({ ...form, industryType: v })}>
              ...
              <SelectContent>
                {['Technology', 'Healthcare', 'Finance', 'Education', 'Manufacturing', 'Retail', 'Real Estate', 'Media', 'Consulting', 'Logistics', 'Other'].map(i => (
          ...
          <div className="space-y-2">
            ...
            <Select value={form.country || 'India'} onValueChange={v => setForm({ ...form, country: v })}>
              <SelectTrigger><SelectValue /></SelectTrigger>
              <SelectContent>
                {['India', 'USA', 'UK', 'Australia', 'Singapore', 'UAE'].map(c => (
                  <SelectItem key={c} value={c}>{c}</SelectItem>
                ))}
              </SelectContent>
            </Select>
          ...
          <div className="space-y-2">
            ...
            <Select value={form.timezone || 'Asia/Kolkata'} onValueChange={v => setForm({ ...form, timezone: v })}>
              <SelectTrigger><SelectValue /></SelectTrigger>
              <SelectContent>
                {['Asia/Kolkata', 'America/New_York', 'America/Los_Angeles', 'Europe/London', 'Asia/Singapore', 'Australia/Sydney'].map(t => (
          ...
          <div className="space-y-2">
            ...
            <Select value={form.defaultLanguage || 'English'} onValueChange={v => setForm({ ...form, defaultLanguage: v })}>
              <SelectTrigger><SelectValue /></SelectTrigger>
              <SelectContent>
                {['English', 'Hindi', 'Marathi', 'Tamil', 'Telugu', 'Kannada'].map(l => (
      ...
      <div className="space-y-4 py-2">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div className="space-y-2">
            ...
            <Input value={form.name || ''} onChange={e => setForm({ ...form, name: e.target.value })} placeholder="e.g. Mumbai HQ" />
          </div>
          <div className="space-y-2">
            <Label>Code *</Label>
            <Input value={form.code || ''} onChange={e => setForm({ ...form, code: e.target.value.toUpperCase() })} placeholder="e.g. MUM-HQ" className="uppercase" />
          </div>
          <div className="space-y-2">
            <Label>Company</Label>
          ...
          <div className="space-y-2">
            ...
            <Input value={form.branchManager || ''} onChange={e => setForm({ ...form, branchManager: e.target.value })} placeholder="Manager name" />
          </div>
          <div className="space-y-2 sm:col-span-2">
            <Label>Address</Label>
            <Textarea value={form.address || ''} onChange={e => setForm({ ...form, address: e.target.value })} placeholder="Full address" rows={2} />
          ...
          <div className="space-y-2">
            ...
            <Input value={form.geoLocation || ''} onChange={e => setForm({ ...form, geoLocation: e.target.value })} placeholder="e.g. 19.0596, 72.8654" />
          </div>
          <div className="space-y-2">
            <Label>Timezone</Label>
            <Select value={form.timezone || 'Asia/Kolkata'} onValueChange={v => setForm({ ...form, timezone: v })}>
              <SelectTrigger><SelectValue /></SelectTrigger>
              <SelectContent>
                {['Asia/Kolkata', 'America/New_York', 'Europe/London', 'Asia/Singapore'].map(t => (
                  <SelectItem key={t} value={t}>{t}</SelectItem>
                ))}
              </SelectContent>
            </Select>
          ...
          <div className="space-y-2">
            ...
            <Input value={form.geofenceRadius || ''} onChange={e => setForm({ ...form, geofenceRadius: e.target.value })} placeholder="e.g. 500m" />
          </div>
          {mode === 'edit' && (
            <div className="space-y-2 flex items-center gap-3">
              <Label>Status</Label>
  ...
  function DepartmentForm({ record, mode }: { record: DepartmentMaster | null; mode: 'add' | 'edit' }) {
    ...
    const handleSave = () => {
      if (!form.name || !form.code) {
        toast({ title: 'Validation Error', description: 'Department Name and Code are required.', variant: 'destructive' })
        return
      }
      if (mode === 'add') {
    ...
    return (
      <div className="space-y-4 py-2">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div className="space-y-2">
            <Label>Code *</Label>
            <Input value={form.code || ''} onChange={e => setForm({ ...form, code: e.target.value.toUpperCase() })} placeholder="e.g. ENG" className="uppercase" />
          </div>
          <div className="space-y-2">
            <Label>Company</Label>
            <Select value={form.company || ''} onValueChange={v => setForm({ ...form, company: v })}>
          ...
          <div className="space-y-2">
            ...
            <Select value={form.parentDepartment || '-'} onValueChange={v => setForm({ ...form, parentDepartment: v })}>
          ...
          <div className="space-y-2">
            ...
            <Input value={form.departmentHead || ''} onChange={e => setForm({ ...form, departmentHead: e.target.value })} placeholder="Head name" />
          </div>
          <div className="space-y-2">
            <Label>Cost Center</Label>
            <Input value={form.costCenter || ''} onChange={e => setForm({ ...form, costCenter: e.target.value })} placeholder="e.g. CC-ENG-001" />
          </div>
          {mode === 'edit' && (
            <div className="space-y-2 flex items-center gap-3">
              <Label>Status</Label>
  ...
  function DesignationForm({ record, mode }: { record: DesignationMaster | null; mode: 'add' | 'edit' }) {
    const [form, setForm] = useState<Partial<DesignationMaster>>(
      mode === 'edit' && record
        ? { ...record }
        : { name: '', code: '', department: '', grade: 'E1', level: 1, jobDescription: '', status: 'active' }
    )

    const handleSave = () => {
      if (!form.name || !form.code) {
        toast({ title: 'Validation Error', description: 'Designation Name and Code are required.', variant: 'destructive' })
        return
      }
      if (mode === 'add') {
        ...
        toast({ title: 'Designation Added', description: `${newRecord.name} has been created.` })
        setAddOpen(false)
      } else if (record) {
        setDesignations(prev => prev.map(d => d.id === record.id ? { ...d, ...form } as DesignationMaster : d))
        toast({ title: 'Designation Updated', description: `${form.name} has been updated.` })
        setEditOpen(false)
        setEditingRecord(null)
      }
    }
      ...
      <div className="space-y-4 py-2">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div className="space-y-2">
            ...
            <Input value={form.name || ''} onChange={e => setForm({ ...form, name: e.target.value })} placeholder="e.g. Software Engineer" />
          </div>
          <div className="space-y-2">
            <Label>Code *</Label>
            <Input value={form.code || ''} onChange={e => setForm({ ...form, code: e.target.value.toUpperCase() })} placeholder="e.g. SE" className="uppercase" />
          </div>
          <div className="space-y-2">
          ...
          <div className="space-y-2">
            <Label>Grade</Label>
            <Select value={form.grade || 'E1'} onValueChange={v => setForm({ ...form, grade: v })}>
              <SelectTrigger><SelectValue /></SelectTrigger>
              <SelectContent>
                {grades.map(g => <SelectItem key={g.id} value={g.code}>{g.name}</SelectItem>)}
              </SelectContent>
            </Select>
          </div>
          <div className="space-y-2">
            <Label>Level</Label>
            <Input type="number" min={1} max={10} value={form.level || 1} onChange={e => setForm({ ...form, level: parseInt(e.target.value) || 1 })} />
          </div>
          <div className="space-y-2 sm:col-span-2">
            <Label>Job Description</Label>
            <Textarea value={form.jobDescription || ''} onChange={e => setForm({ ...form, jobDescription: e.target.value })} placeholder="Describe the role responsibilities" rows={3} />
          </div>
          {mode === 'edit' && (
            <div className="space-y-2 flex items-center gap-3">
              <Label>Status</Label>
        ...
        <DialogFooter>
          <Button variant="outline" onClick={() => { if (mode === 'add') setAddOpen(false); else setEditOpen(false); setEditingRecord(null) }}>Cancel</Button>
          <Button onClick={handleSave} className="bg-emerald-600 hover:bg-emerald-700">
            {mode === 'add' ? 'Create Designation' : 'Update Designation'}
          </Button>
        </DialogFooter>
      </div>
    )
  ...
  function GradeForm({ record, mode }: { record: GradeMaster | null; mode: 'add' | 'edit' }) {
    const [form, setForm] = useState<Partial<GradeMaster>>(
      mode === 'edit' && record
        ? { ...record }
        : { name: '', code: '', salaryBandMin: 0, salaryBandMax: 0, leaveEligibility: '', benefitsEligibility: '', approvalLevel: 1, status: 'active' }
    )

    const handleSave = () => {
      if (!form.name || !form.code) {
        toast({ title: 'Validation Error', description: 'Grade Name and Code are required.', variant: 'destructive' })
        return
      }
      if (mode === 'add') {
        ...
        toast({ title: 'Grade Added', description: `${newRecord.name} has been created.` })
        setAddOpen(false)
      } else if (record) {
        setGrades(prev => prev.map(g => g.id === record.id ? { ...g, ...form } as GradeMaster : g))
        toast({ title: 'Grade Updated', description: `${form.name} has been updated.` })
        setEditOpen(false)
        setEditingRecord(null)
      }
    }
      ...
      <div className="space-y-4 py-2">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div className="space-y-2">
            ...
            <Input value={form.name || ''} onChange={e => setForm({ ...form, name: e.target.value })} placeholder="e.g. E1 - Junior" />
          </div>
          <div className="space-y-2">
            <Label>Code *</Label>
            <Input value={form.code || ''} onChange={e => setForm({ ...form, code: e.target.value.toUpperCase() })} placeholder="e.g. E1" className="uppercase" />
          </div>
          <div className="space-y-2">
            <Label>Salary Band Min (INR)</Label>
            <Input type="number" value={form.salaryBandMin || 0} onChange={e => setForm({ ...form, salaryBandMin: parseInt(e.target.value) || 0 })} />
          </div>
          <div className="space-y-2">
            <Label>Salary Band Max (INR)</Label>
            <Input type="number" value={form.salaryBandMax || 0} onChange={e => setForm({ ...form, salaryBandMax: parseInt(e.target.value) || 0 })} />
          </div>
          <div className="space-y-2">
            <Label>Leave Eligibility</Label>
            <Input value={form.leaveEligibility || ''} onChange={e => setForm({ ...form, leaveEligibility: e.target.value })} placeholder="e.g. 18 days/year" />
          </div>
          <div className="space-y-2">
            <Label>Benefits Eligibility</Label>
            <Input value={form.benefitsEligibility || ''} onChange={e => setForm({ ...form, benefitsEligibility: e.target.value })} placeholder="e.g. Basic Health" />
          </div>
          <div className="space-y-2">
            <Label>Approval Level</Label>
            <Input type="number" min={1} max={10} value={form.approvalLevel || 1} onChange={e => setForm({ ...form, approvalLevel: parseInt(e.target.value) || 1 })} />
          </div>
          {mode === 'edit' && (
            <div className="space-y-2 flex items-center gap-3">
              <Label>Status</Label>
        ...
        <DialogFooter>
          <Button variant="outline" onClick={() => { if (mode === 'add') setAddOpen(false); else setEditOpen(false); setEditingRecord(null) }}>Cancel</Button>
          <Button onClick={handleSave} className="bg-emerald-600 hover:bg-emerald-700">
            {mode === 'add' ? 'Create Grade' : 'Update Grade'}
          </Button>
        </DialogFooter>
      </div>
    )
  ...
  function ShiftForm({ record, mode }: { record: ShiftMaster | null; mode: 'add' | 'edit' }) {
    const [form, setForm] = useState<Partial<ShiftMaster>>(
      mode === 'edit' && record
        ? { ...record }
        : { name: '', startTime: '09:00', endTime: '18:00', breakDuration: '60 min', graceTime: '15 min', lateMarkRule: 'After grace period', overtimeEligible: true, weeklyOff: 'Saturday, Sunday', status: 'active' }
    )
    ...
    const handleSave = () => {
      ...
      if (mode === 'add') {
        ...
        setShifts(prev => [...prev, newRecord])
        toast({ title: 'Shift Added', description: `${newRecord.name} has been created.` })
        setAddOpen(false)
      } else if (record) {
        setShifts(prev => prev.map(s => s.id === record.id ? { ...s, ...form } as ShiftMaster : s))
        toast({ title: 'Shift Updated', description: `${form.name} has been updated.` })
        setEditOpen(false)
        setEditingRecord(null)
      }
    }
      ...
      <div className="space-y-4 py-2">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div className="space-y-2">
            ...
            <Input value={form.name || ''} onChange={e => setForm({ ...form, name: e.target.value })} placeholder="e.g. General Shift" />
          </div>
          <div className="space-y-2">
          ...
          <div className="space-y-2">
            ...
            <Input type="time" value={form.endTime || '18:00'} onChange={e => setForm({ ...form, endTime: e.target.value })} />
          </div>
          <div className="space-y-2">
            <Label>Break Duration</Label>
            <Input value={form.breakDuration || ''} onChange={e => setForm({ ...form, breakDuration: e.target.value })} placeholder="e.g. 60 min" />
          </div>
          <div className="space-y-2">
            <Label>Grace Time</Label>
            <Input value={form.graceTime || ''} onChange={e => setForm({ ...form, graceTime: e.target.value })} placeholder="e.g. 15 min" />
          </div>
          <div className="space-y-2">
            <Label>Late Mark Rule</Label>
            <Input value={form.lateMarkRule || ''} onChange={e => setForm({ ...form, lateMarkRule: e.target.value })} placeholder="e.g. After 15 min grace" />
          </div>
          <div className="space-y-2">
            <Label>Weekly Off</Label>
            <Input value={form.weeklyOff || ''} onChange={e => setForm({ ...form, weeklyOff: e.target.value })} placeholder="e.g. Saturday, Sunday" />
          </div>
          <div className="space-y-2 flex items-center gap-3 pt-6">
            <Switch checked={form.overtimeEligible ?? true} onCheckedChange={v => setForm({ ...form, overtimeEligible: v })} />
        ...
        <DialogFooter>
          <Button variant="outline" onClick={() => { if (mode === 'add') setAddOpen(false); else setEditOpen(false); setEditingRecord(null) }}>Cancel</Button>
          <Button onClick={handleSave} className="bg-emerald-600 hover:bg-emerald-700">
            {mode === 'add' ? 'Create Shift' : 'Update Shift'}
          </Button>
        </DialogFooter>
      </div>
    )
  ...
  function LeaveTypeForm({ record, mode }: { record: LeaveTypeMaster | null; mode: 'add' | 'edit' }) {
    const [form, setForm] = useState<Partial<LeaveTypeMaster>>(
      mode === 'edit' && record
        ? { ...record }
        : { name: '', code: '', paidUnpaid: 'paid', annualQuota: 0, accrualRule: '', carryForwardRule: '', encashmentRule: '', applicableGender: 'All', applicableEmploymentType: 'Full-time', approvalRequired: true, attachmentRequired: false, status: 'active' }
    )

    const handleSave = () => {
      if (!form.name || !form.code) {
        toast({ title: 'Validation Error', description: 'Leave Type Name and Code are required.', variant: 'destructive' })
        return
      }
      if (mode === 'add') {
        ...
        setLeaveTypes(prev => [...prev, newRecord])
        toast({ title: 'Leave Type Added', description: `${newRecord.name} has been created.` })
        setAddOpen(false)
      } else if (record) {
        setLeaveTypes(prev => prev.map(l => l.id === record.id ? { ...l, ...form } as LeaveTypeMaster : l))
    ...
    return (
      <div className="space-y-4 py-2">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div className="space-y-2">
            <Label>Code *</Label>
            <Input value={form.code || ''} onChange={e => setForm({ ...form, code: e.target.value.toUpperCase() })} placeholder="e.g. CL" className="uppercase" />
          </div>
          <div className="space-y-2">
            <Label>Paid/Unpaid</Label>
            <Select value={form.paidUnpaid || 'paid'} onValueChange={v => setForm({ ...form, paidUnpaid: v as 'paid' | 'unpaid' })}>
              <SelectTrigger><SelectValue /></SelectTrigger>
              <SelectContent>
          ...
          <div className="space-y-2">
            ...
            <Input type="number" min={0} value={form.annualQuota || 0} onChange={e => setForm({ ...form, annualQuota: parseInt(e.target.value) || 0 })} />
          </div>
          <div className="space-y-2">
            <Label>Accrual Rule</Label>
            <Input value={form.accrualRule || ''} onChange={e => setForm({ ...form, accrualRule: e.target.value })} placeholder="e.g. 1 per month" />
          </div>
          <div className="space-y-2">
            <Label>Carry Forward Rule</Label>
            <Input value={form.carryForwardRule || ''} onChange={e => setForm({ ...form, carryForwardRule: e.target.value })} placeholder="e.g. Max 3 days" />
          </div>
          <div className="space-y-2">
            <Label>Encashment Rule</Label>
            <Input value={form.encashmentRule || ''} onChange={e => setForm({ ...form, encashmentRule: e.target.value })} placeholder="e.g. Not allowed" />
          </div>
          <div className="space-y-2">
            <Label>Applicable Gender</Label>
            <Select value={form.applicableGender || 'All'} onValueChange={v => setForm({ ...form, applicableGender: v })}>
              <SelectTrigger><SelectValue /></SelectTrigger>
              <SelectContent>
          ...
          <div className="space-y-2">
            <Label>Applicable Employment Type</Label>
            <Select value={form.applicableEmploymentType || 'Full-time'} onValueChange={v => setForm({ ...form, applicableEmploymentType: v })}>
              <SelectTrigger><SelectValue /></SelectTrigger>
              <SelectContent>
                {['All', 'Full-time', 'Part-time', 'Contract', 'Intern'].map(t => <SelectItem key={t} value={t}>{t}</SelectItem>)}
              </SelectContent>
            </Select>
          </div>
          <div className="space-y-2 flex items-center gap-3 pt-6">
            <Switch checked={form.approvalRequired ?? true} onCheckedChange={v => setForm({ ...form, approvalRequired: v })} />
            <Label>Approval Required</Label>
          </div>
          <div className="space-y-2 flex items-center gap-3 pt-6">
            <Switch checked={form.attachmentRequired ?? false} onCheckedChange={v => setForm({ ...form, attachmentRequired: v })} />
  ...
  function SkillForm({ record, mode }: { record: SkillMaster | null; mode: 'add' | 'edit' }) {
    const [form, setForm] = useState<Partial<SkillMaster>>(
      mode === 'edit' && record
        ? { ...record }
        : { name: '', category: '', level: 'Intermediate', certificationRequired: false, status: 'active' }
    )

    const handleSave = () => {
      if (!form.name) {
        toast({ title: 'Validation Error', description: 'Skill Name is required.', variant: 'destructive' })
        return
      }
      if (mode === 'add') {
        ...
        toast({ title: 'Skill Added', description: `${newRecord.name} has been created.` })
        setAddOpen(false)
      } else if (record) {
        setSkills(prev => prev.map(s => s.id === record.id ? { ...s, ...form } as SkillMaster : s))
        toast({ title: 'Skill Updated', description: `${form.name} has been updated.` })
        setEditOpen(false)
        setEditingRecord(null)
      }
    }
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div className="space-y-2">
            ...
            <Select value={form.category || ''} onValueChange={v => setForm({ ...form, category: v })}>
              ...
              <SelectContent>
                {['Frontend', 'Backend', 'Database', 'Cloud', 'DevOps', 'AI/ML', 'Analytics', 'Management', 'Soft Skills'].map(c => (
                  <SelectItem key={c} value={c}>{c}</SelectItem>
                ))}
              </SelectContent>
            </Select>
          ...
          <div className="space-y-2">
            <Label>Level</Label>
            <Select value={form.level || 'Intermediate'} onValueChange={v => setForm({ ...form, level: v })}>
              <SelectTrigger><SelectValue /></SelectTrigger>
              <SelectContent>
          ...
          <div className="space-y-2 flex items-center gap-3 pt-6">
            <Switch checked={form.certificationRequired ?? false} onCheckedChange={v => setForm({ ...form, certificationRequired: v })} />
  ...
  function DocumentTypeForm({ record, mode }: { record: DocumentTypeMaster | null; mode: 'add' | 'edit' }) {
    const [form, setForm] = useState<Partial<DocumentTypeMaster>>(
      mode === 'edit' && record
        ? { ...record }
        : { name: '', mandatoryOptional: 'mandatory', applicableModule: 'Onboarding', expiryRequired: false, verificationRequired: true, status: 'active' }
    )
    ...
    const handleSave = () => {
      ...
      if (mode === 'add') {
        ...
        setDocumentTypes(prev => [...prev, newRecord])
        toast({ title: 'Document Type Added', description: `${newRecord.name} has been created.` })
        setAddOpen(false)
      } else if (record) {
        setDocumentTypes(prev => prev.map(d => d.id === record.id ? { ...d, ...form } as DocumentTypeMaster : d))
        toast({ title: 'Document Type Updated', description: `${form.name} has been updated.` })
        setEditOpen(false)
        setEditingRecord(null)
      }
    }
      ...
      <div className="space-y-4 py-2">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div className="space-y-2">
            ...
            <Input value={form.name || ''} onChange={e => setForm({ ...form, name: e.target.value })} placeholder="e.g. Offer Letter" />
          </div>
          <div className="space-y-2">
            <Label>Mandatory/Optional</Label>
            <Select value={form.mandatoryOptional || 'mandatory'} onValueChange={v => setForm({ ...form, mandatoryOptional: v as 'mandatory' | 'optional' })}>
              <SelectTrigger><SelectValue /></SelectTrigger>
              <SelectContent>
          ...
          <div className="space-y-2">
            ...
            <Select value={form.applicableModule || 'Onboarding'} onValueChange={v => setForm({ ...form, applicableModule: v })}>
              <SelectTrigger><SelectValue /></SelectTrigger>
              <SelectContent>
                {['Onboarding', 'Payroll', 'Attendance', 'Employee', 'Compliance', 'General'].map(m => (
          ...
          <div className="space-y-2 flex items-center gap-3 pt-6">
            <Switch checked={form.expiryRequired ?? false} onCheckedChange={v => setForm({ ...form, expiryRequired: v })} />
            <Label>Expiry Required</Label>
          </div>
          <div className="space-y-2 flex items-center gap-3 pt-6">
            <Switch checked={form.verificationRequired ?? true} onCheckedChange={v => setForm({ ...form, verificationRequired: v })} />
        ...
        <DialogFooter>
          <Button variant="outline" onClick={() => { if (mode === 'add') setAddOpen(false); else setEditOpen(false); setEditingRecord(null) }}>Cancel</Button>
          <Button onClick={handleSave} className="bg-emerald-600 hover:bg-emerald-700">
            {mode === 'add' ? 'Create Document Type' : 'Update Document Type'}
          </Button>
        </DialogFooter>
      </div>
    )
