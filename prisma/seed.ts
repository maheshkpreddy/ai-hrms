...
async function main() {
  ...
  const deleteOps = [
    () => prisma.subVendorResume.deleteMany(),
  ...
  const marq = await prisma.company.create({ data: { name: 'MARQ AI Technologies', code: 'MARQ', industry: 'AI & Technology', country: 'IN', currency: 'INR', timezone: 'Asia/Kolkata', domain: 'marqai.com', isActive: true } });
  ...
  const marqDepts = await Promise.all(['Engineering', 'HR', 'Finance', 'Marketing', 'Sales', 'Operations'].map(n => prisma.department.create({ data: { name: n, code: `MQ-${n.substring(0, 3).toUpperCase()}`, description: `${n} Department`, isActive: true, companyId: marq.id } })));
  console.log('✅ MARQ Departments:', marqDepts.length);

  // ═══════════════════════ COMPANY 2: TechCorp Global ═══════════════════════
  const tcg = await prisma.company.create({ data: { name: 'TechCorp Global', code: 'TCGC', industry: 'IT Services', country: 'US', currency: 'USD', timezone: 'America/Los_Angeles', domain: 'techcorp.com', isActive: true } });
  console.log('✅ Company:', tcg.name);

  const tcgBranches = await Promise.all([
    prisma.branch.create({ data: { name: 'San Francisco HQ', code: 'TC-SF', city: 'San Francisco', state: 'CA', country: 'US', address: '101 Market St, San Francisco', isActive: true, companyId: tcg.id } }),
    prisma.branch.create({ data: { name: 'New York Office', code: 'TC-NY', city: 'New York', state: 'NY', country: 'US', address: '350 5th Ave, New York', isActive: true, companyId: tcg.id } }),
  ...
  const tcgDepts = await Promise.all(['Engineering', 'HR', 'Finance', 'Marketing', 'Sales', 'Operations'].map(n => prisma.department.create({ data: { name: n, code: `TC-${n.substring(0, 3).toUpperCase()}`, description: `${n} Department`, isActive: true, companyId: tcg.id } })));
  ...
  // ═══════════════════════ USERS & EMPLOYEES ═══════════════════════
  const marqAdminUser = await prisma.user.create({ data: { email: 'admin@marqai.com', password: hashedPassword, name: 'MARQ Admin', role: 'super_admin', isActive: true, companyId: marq.id } });
  const tcgAdminUser = await prisma.user.create({ data: { email: 'admin@techcorp.com', password: hashedPassword, name: 'TCG Admin', role: 'super_admin', isActive: true, companyId: tcg.id } });
  const platformAdmin = await prisma.user.create({ data: { email: 'superadmin@eh2r.com', password: hashedPassword, name: 'Platform Super Admin', role: 'super_admin', isActive: true } });
  ...
  for (const [compId, empDefs, depts, branches] of [[marq.id, marqEmpDefs, marqDepts, marqBranches], [tcg.id, tcgEmpDefs, tcgDepts, tcgBranches]] as const) {
    for (const ed of empDefs) {
      const user = await prisma.user.create({ data: { email: ed.email, password: hashedPassword, name: `${ed.fn} ${ed.ln}`, role: ed.role, isActive: true, companyId: compId } });
      allUsers.push(user);
  ...
  const marqJobs = await Promise.all([
    ...
    prisma.job.create({ data: { title: 'AI/ML Research Scientist', description: 'Work on cutting-edge AI research and model development.', requirements: 'ML, Python, TensorFlow, 4+ years', department: 'Engineering', location: 'Hyderabad', employmentType: 'Full-time', experienceMin: 4, experienceMax: 7, salaryMin: 2000000, salaryMax: 3500000, status: 'open', priority: 'urgent', positions: 1, postedDate: daysAgo(15), companyId: marq.id } }),
  ...
  for (let i = 0; i < 2; i++) {
    ...
    await prisma.aIInterview.create({ data: { candidateId: cand.id, jobId: cand.jobId, status: isCompleted ? 'completed' : 'in_progress', questions: JSON.stringify(aiQuestions), responses: JSON.stringify(responses), score: isCompleted ? overall : null, feedback: isCompleted ? JSON.stringify({ overallScore: overall, recommendation: overall >= 75 ? 'proceed' : 'reject' }) : null, language: 'en', interviewLink: `/interview/ai-marq-${i + 1}`, cvScore: 70 + i * 8, duration: isCompleted ? 22 + i * 5 : null, startedAt: daysAgo(5 - i), completedAt: isCompleted ? daysAgo(4 - i) : null } });
  }
  for (let i = 0; i < 2; i++) {
    ...
    const overall = Math.round(scores.reduce((a, b) => a + b, 0) / scores.length);
    const isCompleted = i === 0;
    await prisma.aIInterview.create({ data: { candidateId: cand.id, jobId: cand.jobId, status: isCompleted ? 'completed' : 'in_progress', questions: JSON.stringify(aiQuestions), responses: JSON.stringify(responses), score: isCompleted ? overall : null, feedback: isCompleted ? JSON.stringify({ overallScore: overall, recommendation: overall >= 75 ? 'proceed' : 'reject' }) : null, language: 'en', interviewLink: `/interview/ai-tcg-${i + 1}`, cvScore: 72 + i * 6, duration: isCompleted ? 25 + i * 4 : null, startedAt: daysAgo(3 - i), completedAt: isCompleted ? daysAgo(2 - i) : null } });
  }
  ...
  for (let dayOffset = 0; dayOffset < 7; dayOffset++) {
    ...
    for (const emp of allEmployees) {
      ...
      if (status !== 'absent') {
        await prisma.attendance.create({ data: { date, checkIn: d(`${dateStr}T${status === 'late' ? '10' : '09'}:${String(Math.floor(Math.random() * 30)).padStart(2, '0')}:00`), checkOut: status === 'half_day' ? d(`${dateStr}T13:00:00`) : d(`${dateStr}T18:${String(Math.floor(Math.random() * 30)).padStart(2, '0')}:00`), workHours: status === 'half_day' ? 4 : 8, status, source: 'web', gpsLatitude: coords.lat + latVariation, gpsLongitude: coords.lng + lngVariation, employeeId: emp.id } });
      } else {
        await prisma.attendance.create({ data: { date, status, source: 'web', gpsLatitude: coords.lat + latVariation, gpsLongitude: coords.lng + lngVariation, employeeId: emp.id } });
      }
      attCount++;
    }
  }
  ...
  // ═══════════════════════ TICKETS ═══════════════════════
  for (const comp of [marq, tcg]) { const compEmps = allEmployees.filter(e => e.companyId === comp.id); for (const [subject, cat, pri, status] of [['VPN Connection Issue', 'it', 'high', 'in_progress'], ['Payroll Discrepancy', 'payroll', 'urgent', 'open'], ['Access Request', 'it', 'medium', 'resolved'], ['New Laptop Request', 'it', 'medium', 'open'], ['Cafeteria Feedback', 'general', 'low', 'resolved']] as const) { await prisma.ticket.create({ data: { subject, description: `Detailed: ${subject}`, category: cat, priority: pri, status, resolution: status === 'resolved' ? 'Resolved successfully' : null, employeeId: compEmps[0].id } }); } }
  console.log('✅ Tickets: 10');

  // ═══════════════════════ HELPDESK ═══════════════════════
  for (const [comp, prefix] of [[marq, 'MARQ'], [tcg, 'TCG']] as const) { const compEmps = allEmployees.filter(e => e.companyId === comp.id); for (let i = 0; i < 5; i++) { const [subject, cat, pri, status] = [['Email not syncing', 'IT', 'high', 'open'], ['License renewal', 'IT', 'medium', 'in_progress'], ['AC not working', 'Facilities', 'medium', 'resolved'], ['New hire equipment', 'IT', 'high', 'open'], ['Badge access issue', 'Security', 'urgent', 'resolved']][i] as const; const ticket = await prisma.helpdeskTicket.create({ data: { ticketId: `${prefix}-HD-${String(i + 1001).padStart(4, '0')}`, requesterId: compEmps[i % compEmps.length].id, requesterType: 'employee', category: cat, priority: pri, subject, description: `Detailed: ${subject}`, status, resolution: status === 'resolved' ? 'Resolved.' : null, companyId: comp.id } }); await prisma.helpdeskTicketComment.create({ data: { content: status === 'resolved' ? 'Fixed.' : 'Looking into it.', ticketId: ticket.id, employeeId: compEmps[1].id } }); } }
  console.log('✅ Helpdesk: 10');

  // ═══════════════════════ CLIENTS (3 per company) ═══════════════════════
  for (const cd of [{ name: 'TechCorp Solutions', email: 'biz@techcorp.in', industry: 'IT' }, { name: 'Global Finance', email: 'partner@globalfin.in', industry: 'Finance' }, { name: 'HealthFirst India', email: 'connect@healthfirst.in', industry: 'Healthcare' }]) { await prisma.client.create({ data: { ...cd, clientCompany: cd.name, contractStart: d('2024-01-01'), contractEnd: d('2025-12-31'), status: 'active', companyId: marq.id } }); }
  for (const cd of [{ name: 'Acme Corp', email: 'hr@acme.com', industry: 'Technology' }, { name: 'GlobalTech Inc', email: 'talent@globaltech.com', industry: 'Software' }, { name: 'BuildRight', email: 'biz@buildright.com', industry: 'Construction' }]) { await prisma.client.create({ data: { ...cd, clientCompany: cd.name, contractStart: d('2024-01-01'), contractEnd: d('2025-12-31'), status: 'active', companyId: tcg.id } }); }
  console.log('✅ Clients: 6');

  // ═══════════════════════ VENDORS & SUBVENDORS ═══════════════════════
  for (const [comp, vDefs] of [[marq, [{ name: 'TalentHunt India', email: 'hr@talenthunt.in', vendorCompany: 'TalentHunt', serviceType: 'Recruitment', rating: 4.5 }, { name: 'CloudHost India', email: 'support@cloudhost.in', vendorCompany: 'CloudHost', serviceType: 'IT Infra', rating: 4.0 }, { name: 'SecureIT', email: 'contact@secureit.in', vendorCompany: 'SecureIT', serviceType: 'Cybersecurity', rating: 4.2 }]], [tcg, [{ name: 'TalentHunt Agency', email: 'info@talenthunt.com', vendorCompany: 'TalentHunt', serviceType: 'recruitment', rating: 4.5 }, { name: 'StaffPro Solutions', email: 'contact@staffpro.com', vendorCompany: 'StaffPro', serviceType: 'staffing', rating: 4.2 }, { name: 'VerifyRight BGV', email: 'team@verifyright.com', vendorCompany: 'VerifyRight', serviceType: 'bgv', rating: 4.8 }]]] as const) { for (const vd of vDefs) { const vendor = await prisma.vendor.create({ data: { name: vd.name, email: vd.email, vendorCompany: vd.vendorCompany, serviceType: vd.serviceType, rating: vd.rating, status: 'active', companyId: comp.id } }); await prisma.subVendor.create({ data: { companyName: `${vd.vendorCompany} Associates`, contactPerson: `${vd.name} Partner`, email: `partner@${vd.email.split('@')[1]}`, status: 'active', vendorId: vendor.id } }); } }
  console.log('✅ Vendors: 6, SubVendors: 6');

  // ═══════════════════════ DOCUMENTS (5 per company) ═══════════════════════
  for (const comp of [marq, tcg]) { const compEmps = allEmployees.filter(e => e.companyId === comp.id); for (const [name, type] of [['Employment Offer Letter', 'contract'], ['Non-Disclosure Agreement', 'legal'], ['ID Proof', 'id_proof'], ['Tax Form', 'tax'], ['Benefits Enrollment Form', 'general']] as const) { await prisma.document.create({ data: { name, title: `${name} - ${compEmps[0].employeeId}`, type, status: 'active', employeeId: compEmps[0].id } }); } }
  ...
  console.log('  - MARQ Admin: admin@marqai.com / admin123');
  console.log('  - TCG Admin: admin@techcorp.com / admin123');
  console.log('  - All employee passwords: admin123');
}
